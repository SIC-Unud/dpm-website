      ...
      <div class="col-12">
        <div class="card shadow mb-4">
          ...
          <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            ...
            <button class="btn btn-primary" data-toggle="modal" data-target="#postForm" id="BtnCreate"
              data-service="create">Add Post</button>
          </div>
          <!-- Card Body -->
          <div class="card-body">
            @if ($posts)
                ...
                @foreach ($posts as $post)
                  <div class="list-group-item list-group-item-action flex-column align-items-start">
                    <div class="d-flex w-100 justify-content-between">
                      ...
                      <div class="d-flex">
                        {{-- @if ($post->image)
                                                    <a href="{{ $post->image }}" class="btn btn-primary mr-3"
                                                        target="_blank">View Image</a>
                                                @endif --}}
                        <button class="btn btn-primary button-edit mr-3" data-id="{{ $post->id }}" data-service="edit"
                          data-toggle="modal" data-target="#postForm">Edit</button>
              ...
              <form method="POST" id="formData">
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="link" id="link" class="form-control" placeholder="input Link ..."
                      value="">
                  </div>
                  {{-- <div class="form-group">
...
@section('scripts')
  <script type="text/javascript">
    ...
    console.log("{{ route('admin.post.store') }}");
    ...
    $(".button-edit").click(function(e) {
      ...
      for (i = 0; i < postData.length; i++) {
        if (postData[i]['id'] == $(this).data('id')) {
          ['title', 'link'].forEach(function(name) {
            $("#formData input[name='" + name + "']").val(postData[i][name]);
            $("#formData textarea[name='" + name + "']").val(postData[i][name]);
          });
          break;
        }
      }
